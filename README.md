...
Windows-Hello-style IR face authentication for [Omarchy](https://omarchy.org), covering `sudo`, polkit, and the lock screen. Backed by [howdy-next](https://github.com/LonerTZ/howdy-next) (an OpenCV YuNet/SFace matcher and PAM module) pointed at an IR camera instead of your regular webcam.
...
**2. Plain CLI scripts in `scripts/`** that the Omarchy plugin system never executes on its own — `omarchy-hw-ir-camera`, `omarchy-setup-security-face`, `omarchy-remove-security-face`. Adding or enabling the plugin does not run these. You have to consciously run `scripts/install.sh`, which installs root-owned copies into Omarchy's command directory and `/usr/bin` so `omarchy setup security face` and `omarchy remove security face` become real, discoverable `omarchy` subcommands (the same mechanism `omarchy-setup-security-fingerprint` uses). Nothing is enrolled, no package is built, and no PAM file is touched by `install.sh` itself. It does install a root-owned, hash-pinned Howdy build recipe under `/usr/share/omarchy-face-auth/` so the later privileged setup command never executes packaging instructions from the user-writable plugin checkout.
...
2. **`bash scripts/install.sh`** — installs the three scripts as root-owned `0755` files in Omarchy's command directory and `/usr/bin` (`sudo install`, one prompt), so `omarchy setup security face` / `omarchy remove security face` exist as commands. They are copies rather than links into the user-writable checkout; after updating or editing the checkout, rerun `install.sh` to install the new command bytes. The installer also checks `~/.config/omarchy/plugins/io.github.pb3975.face-auth`: if `omarchy plugin add` already made that a real directory, it's left alone; if it doesn't exist yet, `install.sh` symlinks the checkout in itself as a dev-style shortcut. Either way, still no PAM edits, no Howdy build, and no enrollment.
3. **`omarchy setup security face`** — the interactive part. Requires sudo. In order: installs `v4l-utils` if missing, runs `omarchy-hw-ir-camera` and bails with an error if no IR camera is found, builds `howdy-next` **3.4.0** from upstream commit `b0b3d290cbd38a0a44f6bf0a37b414210acf8276` using the root-owned PKGBUILD installed with this plugin, and rejects the upstream source archive unless its SHA-256 is `bfd1e5f3c379bb99e5df13ac086e1e4b90c07f4f474131293327a84346428144`. Build/runtime dependencies come from pacman's signed official Arch repositories; no AUR PKGBUILD is executed. It then runs `sudo howdy download-models` and independently rechecks the exact YuNet/SFace model byte sizes and SHA-256 values embedded by that pinned Howdy release before enrollment continues. Finally it points Howdy's `device_path` at the detected IR node in `/etc/howdy/config.ini`, walks you through `sudo howdy add` and `sudo howdy test`, and **only if both succeed** writes PAM config. A failed source/model verification, enrollment, or face test aborts before PAM is enabled.
...
- `/etc/systemd/system/polkit-agent-helper@.service.d/50-howdy-camera.conf` — packaged with the pinned `howdy-next` build. polkit 127 isolates its PAM helper from `/dev/video*`; this drop-in disables the private device namespace for that helper and limits the added device allowance to `char-video4linux rw`. Removing `howdy-next` removes the drop-in and the removal command reloads systemd.
...
All three PAM edits are idempotent (`grep -q` before `sed -i`) and use `sufficient`, never `requisite` — see [Security](#security). Removal rewrites existing PAM files entirely inside one privileged process: root creates the candidate beside the target, validates that the Howdy lines are gone, preserves the original file metadata, and publishes it with an atomic same-filesystem rename.
...
- **Supply-chain inputs are pinned.** The plugin does not execute the mutable AUR `howdy-next` recipe. Its root-owned PKGBUILD verifies the exact upstream v3.4.0 source archive by SHA-256 before compiling. Howdy's two OpenCV model URLs are commit-pinned, Howdy verifies their hashes/sizes before publishing them, and the setup script independently verifies the same model hashes/sizes again. `.github/workflows/security-pins.yml` downloads those exact inputs on every push/PR so drift fails CI.
...
packaging/howdy-next/PKGBUILD         pinned upstream Howdy source build recipe
packaging/howdy-next/polkit-camera.conf bounded polkit >=127 camera-access drop-in
.github/workflows/security-pins.yml   verifies source/model pins and shell syntax
